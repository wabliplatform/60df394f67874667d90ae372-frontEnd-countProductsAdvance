let apiSupplierApi = new TempApi.SupplierApi();import TempApi from '../src/index';let apiProductApi = new TempApi.ProductApi();let product = new TempApi.Product();document.getElementById('ir0lo').onclick = (event) => {
...
document.addEventListener('alignpSupplier', function(e) {
  ...
  [...advanceSelect.querySelectorAll("[annotationname]")].forEach(
    ...
    product['pImage'] = {
      ...
      product['pTitle'] = document.querySelector("[annotationname = 'pTitle']").value;product['pCount'] = document.querySelector("[annotationname = 'pCount']").value;product['pSupplier'] = document.querySelector("[annotationname = 'pSupplier']").value;apiProductApi.createproduct( product, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); {  location.href= '/Dashboard/'+response.body.query._id+'' ;}}});};window.onload = () => {apiSupplierApi.getAllsupplier((error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("iyqn5").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
